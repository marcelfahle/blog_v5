...
async function generateOgImage({ slug, title, description }) {
  ...
  const publishDir = path.join(process.cwd(), '_site', 'img', 'og', 'generated');
  const publishFile = path.join(publishDir, `${safeSlug}.png`);

  try {
    await ensureDirectory(OUTPUT_DIR);
    await ensureDirectory(publishDir);

    if (fs.existsSync(outputFile)) {
      // Ensure it also exists in the publish directory (in case of clean build)
      await fs.promises.copyFile(outputFile, publishFile);
      return outputFile;
    }
    ...
    await fs.promises.writeFile(publishFile, pngBuffer);
